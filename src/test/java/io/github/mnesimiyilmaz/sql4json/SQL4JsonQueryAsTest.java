...
class SQL4JsonQueryAsTest {
    ...
    void when_queryAsList_then_each_row_mapped() {
        List<Person> people = SQL4Json.queryAsList("SELECT name, age FROM $r", JSON, Person.class);
        assertEquals(List.of(new Person("Alice", 30), new Person("Bob", 40)), people);
    }

    @Test
    void when_queryAs_with_single_row_result_then_unwrap() {
        Person p = SQL4Json.queryAs("SELECT name, age FROM $r WHERE age = 30", JSON, Person.class);
    ...
    void when_queryAs_with_zero_rows_then_exception() {
        assertThrows(
                ...
                () -> SQL4Json.queryAs("SELECT name, age FROM $r WHERE age = 999", JSON, Person.class));
    }

    @Test
    void when_queryAs_with_multiple_rows_then_exception() {
        SQL4JsonMappingException e = assertThrows(
                SQL4JsonMappingException.class, () -> SQL4Json.queryAs("SELECT name, age FROM $r", JSON, Person.class));
    ...
    void when_join_query_as_list_then_mapped() {
        ...
        assertEquals(
                java.util.Set.of(new UserOrder("Alice", 100), new UserOrder("Bob", 200)), java.util.Set.copyOf(result));
    ...
    void when_queryAsList_with_empty_result_then_empty_list() {
        List<Person> out = SQL4Json.queryAsList("SELECT name, age FROM $r WHERE age = 999", JSON, Person.class);
    ...
    void when_queryAsJsonValue_with_empty_dataSources_map_then_exception() {
        assertThrows(SQL4JsonException.class, () -> SQL4Json.queryAsJsonValue("SELECT * FROM users", Map.of()));
    }
}
