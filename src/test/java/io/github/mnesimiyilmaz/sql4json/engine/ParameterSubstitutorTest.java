...
class ParameterSubstitutorTest {
    ...
    void when_positional_param_index_out_of_range_then_exception() {
        ...
        assertThrows(SQL4JsonBindException.class, () -> ParameterSubstitutor.substituteExpression(ref, params, S));
    }
    ...
    void when_simple_case_contains_parameter_ref_then_replaced() {
        ...
        var whenClause = new Expression.WhenClause.ValueWhen(ref, new Expression.LiteralVal(new SqlString("match")));
        var sc = new Expression.SimpleCaseWhen(
                new Expression.ColumnRef("col"), List.of(whenClause), new Expression.LiteralVal(new SqlString("nope")));
    ...
    void when_top_level_substitute_without_params_returns_def_unchanged() {
        ...
        var def = io.github.mnesimiyilmaz.sql4json.parser.QueryParser.parse("SELECT * FROM $r WHERE age > 25", S);
    ...
    void when_where_has_parameter_then_substituted_definition_is_parameterless() {
        var def = io.github.mnesimiyilmaz.sql4json.parser.QueryParser.parse("SELECT * FROM $r WHERE age > ?", S);
    ...
    void when_where_has_named_parameter_then_resolved_and_tested_successfully() {
        ...
        var def = io.github.mnesimiyilmaz.sql4json.parser.QueryParser.parse("SELECT * FROM $r WHERE dept = :d", S);
    ...
    void when_limit_and_offset_params_then_resolved_to_integers() {
        var def = io.github.mnesimiyilmaz.sql4json.parser.QueryParser.parse("SELECT * FROM $r LIMIT ? OFFSET ?", S);
    ...
    void when_limit_bigInteger_exceeds_long_then_exception() {
        var def = io.github.mnesimiyilmaz.sql4json.parser.QueryParser.parse("SELECT * FROM $r LIMIT ?", S);
        ...
        SQL4JsonBindException ex = assertThrows(
                SQL4JsonBindException.class, () -> ParameterSubstitutor.substitute(def, BoundParameters.of(huge), S));
    ...
    void when_limit_bigDecimal_exceeds_long_then_exception() {
        var def = io.github.mnesimiyilmaz.sql4json.parser.QueryParser.parse("SELECT * FROM $r LIMIT ?", S);
        java.math.BigDecimal huge = new java.math.BigDecimal(Long.MAX_VALUE).add(java.math.BigDecimal.TEN);
        SQL4JsonBindException ex = assertThrows(
                SQL4JsonBindException.class, () -> ParameterSubstitutor.substitute(def, BoundParameters.of(huge), S));
        assertTrue(ex.getMessage().contains("exceeds Integer.MAX_VALUE"));
    }
    ...
    void when_collection_bound_to_scalar_placeholder_then_exception() {
        var def = io.github.mnesimiyilmaz.sql4json.parser.QueryParser.parse("SELECT * FROM $r WHERE age = :a", S);
        SQL4JsonBindException ex = assertThrows(
    ...
    void when_array_bound_to_scalar_placeholder_then_exception() {
        ...
        SQL4JsonBindException ex = assertThrows(
                SQL4JsonBindException.class,
                () -> ParameterSubstitutor.substitute(def, BoundParameters.of(new int[] {1, 2}), S));
        assertTrue(ex.getMessage().contains("Cannot bind collection"));
    }
    ...
    void when_between_lower_bound_is_now_then_expression_path() {
        ...
        var out = ParameterSubstitutor.substitute(
                def, BoundParameters.named().bind("u", java.time.LocalDateTime.now()), S);
        assertNotNull(out.whereClause());
    }
