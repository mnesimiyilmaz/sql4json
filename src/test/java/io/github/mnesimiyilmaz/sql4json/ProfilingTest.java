 ...
 *
 * <p>Runs a representative set of queries — including JOIN and window functions — against 7 JSON file sizes
 * (8/16/32/64/128/256/512 MB). Captures wall-clock, thread CPU time, user time, heap before/after/delta, and peak heap
 * per run via JMX. Emits a markdown report at {@code target/profiling/profiling-report-<timestamp>.md} plus live stdout
 * output.
 *
 * <p>Tiering: the full scenario list runs against 8/16/32/64 MB. A curated subset (one representative per query shape)
 * runs against 128/256/512 MB to keep total runtime reasonable while still drawing a scaling curve at the heavy sizes.
 *
 * <p><b>Single-threaded measurement.</b> Each scenario is executed end-to-end on the calling thread; SQL4Json does not
 * parallelize a single query internally, so the published numbers reflect single-thread throughput. Multi-core scaling
 * is achieved by running independent queries concurrently from application code, which is out of scope here.
 *
 * <p>Usage:
 *
 * <pre>
...
class ProfilingTest {
    ...
    private static final List<MemoryPoolMXBean> HEAP_POOLS = ManagementFactory.getMemoryPoolMXBeans().stream()
    ...
    private record QueryScenario(String label, String sql, Tier tier, boolean isJoin, ResultAssertion assertion) {}
    ...
    void setup() {
        ...
        assertTrue(
                countryLookupJson.startsWith("[") && countryLookupJson.endsWith("]"),
                "country lookup must be a JSON array");
        assertTrue(
                countryLookupJson.length() > 500,
        ...
        System.out.printf(
                "  %d scenarios loaded; tiering: FULL={8,16,32,64}MB, CURATED={128,256,512}MB%n", SCENARIOS.size());
        System.out.println("═══════════════════════════════════════════════════════════════");
    }
    ...
    private RunMetrics runOne(QueryScenario scenario, String jsonData, String sizeLabel) {
        ...
        try {
            result = scenario.isJoin()
                    ...
                    : SQL4Json.queryAsJsonValue(scenario.sql(), jsonData, LargeTestSettings.INSTANCE);
        } catch (Throwable t) {
        ...
        long peakHeapBytes =
                HEAP_POOLS.stream().mapToLong(p -> p.getPeakUsage().getUsed()).sum();
    ...
    private static List<QueryScenario> buildScenarios() {
        ...
        // ── 2. Projection ───────────────────────────────────────
        s.add(new QueryScenario(
                "p04 PROJECT-TOP-LEVEL", "SELECT id, is_active, score FROM $r", Tier.HEADLINE, false, r -> {
        ...
        s.add(new QueryScenario(
                "p08 GROUP-BY-COUNTRY-MULTI-AGG",
        ...
        s.add(new QueryScenario(
                "p37 COMPLEX-NESTED-GROUP-HAVING",
        ...
        s.add(new QueryScenario(
                "p40 SUBQUERY-GROUP-AFTER-FILTER",
        ...
        s.add(new QueryScenario(
                "p47 STRESS-HIGH-CARDINALITY-GROUP",
        ...
        s.add(new QueryScenario(
                "p53 JOIN-INNER-FILTER",
                "SELECT u.id, u.score, c.region " + "FROM users u JOIN countries c ON u.address.country = c.code "
                        + "WHERE u.score > 70",
                Tier.FULL,
                true,
                r -> assertTrue(r.isArray())));
        ...
        s.add(new QueryScenario(
                "p55 JOIN-INNER-ORDERBY-LIMIT",
                "SELECT u.id, u.score, c.name " + "FROM users u JOIN countries c ON u.address.country = c.code "
        ...
        s.add(new QueryScenario(
                "p56 JOIN-LEFT-ANTI",
                "SELECT u.id, u.address.country " + "FROM users u LEFT JOIN countries c ON u.address.country = c.code "
        ...
        s.add(new QueryScenario(
                "p65 FUNC-NESTED-LPAD-TRIM-COALESCE",
    ...
    private void writeReport() throws IOException {
        ...
        md.append("- Total RAM: ")
                .append(osBean.getTotalMemorySize() / (1024 * 1024))
                .append(" MB\n");
        md.append("- Max heap (-Xmx): ").append(rt.maxMemory() / (1024 * 1024)).append(" MB\n");
        md.append("- Initial heap (-Xms): ")
                .append(rt.totalMemory() / (1024 * 1024))
                .append(" MB (approx, post-warmup)\n");
        md.append("- Profiling runs: ")
        ...
        for (SizeRun sr : allRuns) {
            md.append("| ")
                    .append(sr.sizeLabel)
                    .append(" | ")
                    .append(String.format("%,d", sr.bytes))
                    .append(" | ")
                    .append(String.format("%,d", sr.loadMs))
                    .append(" |\n");
        }
        md.append('\n');
        ...
        if (!headlineLabels.isEmpty()) {
            ...
            appendMetricTable(md, "Wall time (ms)", allRuns, RunMetrics::wallMs, headlineLabels::contains);
            appendMetricTable(md, "Rows out (latest run)", allRuns, RunMetrics::rows, headlineLabels::contains);
        }
        ...
        Files.writeString(outFile, md.toString(), StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING);
        System.out.println("Report: " + outFile.toAbsolutePath());
    }

    private void appendMetricTable(
            StringBuilder md, String title, List<SizeRun> tier, ToLongFunction<RunMetrics> metric) {
    ...
    private void appendMetricTable(
        ...
        for (String label : labels) {
            ...
            for (SizeRun sr : tier) {
                ...
                if (runs.length == 0) {
                    boolean anyFailed =
                            sr.results.stream().anyMatch(m -> m.label().equals(label) && m.error() != null);
                    cell = anyFailed ? "FAIL" : "—";
                } else {
