...
class ParameterBindingIntegrationTest {
    ...
    void when_named_parameters_bound_then_results_filtered() {
        PreparedQuery q = SQL4Json.prepare("SELECT name FROM $r WHERE age > :minAge AND dept = :dept");
        String r = q.execute(JSON, BoundParameters.named().bind("minAge", 26).bind("dept", "Eng"));
    ...
    void when_positional_parameters_via_varargs_then_works() {
        PreparedQuery q = SQL4Json.prepare("SELECT name FROM $r WHERE age > ? AND dept = ?");
    ...
    void when_same_named_used_multiple_times_then_single_bind_is_enough() {
        PreparedQuery q = SQL4Json.prepare("SELECT name FROM $r WHERE dept = :d OR dept = :d");
    ...
    void when_execute_called_with_bound_parameters_against_JsonValue_works() {
        var codec = io.github.mnesimiyilmaz.sql4json.settings.Sql4jsonSettings.defaults()
                .codec();
    ...
    void when_executeAsList_with_parameters_then_mapped() {
        PreparedQuery q = SQL4Json.prepare("SELECT name, age FROM $r WHERE age > :min");
        java.util.List<Person> out =
                q.executeAsList(JSON, Person.class, BoundParameters.named().bind("min", 26));
        assertEquals(java.util.List.of(new Person("Alice", 30), new Person("Bob", 40)), out);
    }

    @Test
    void when_executeAs_single_row_with_parameters_then_mapped() {
        PreparedQuery q = SQL4Json.prepare("SELECT name, age FROM $r WHERE age = :age");
        Person p = q.executeAs(JSON, Person.class, BoundParameters.named().bind("age", 30));
    ...
    void when_executeAsList_with_parameters_against_jsonvalue_then_mapped() {
        var codec = io.github.mnesimiyilmaz.sql4json.settings.Sql4jsonSettings.defaults()
                .codec();
        var data = codec.parse(JSON);
        PreparedQuery q = SQL4Json.prepare("SELECT name, age FROM $r WHERE dept = :d");
        java.util.List<Person> out =
                q.executeAsList(data, Person.class, BoundParameters.named().bind("d", "Eng"));
        assertEquals(java.util.List.of(new Person("Alice", 30), new Person("Carol", 25)), out);
    }
    ...
    void when_executeAs_with_parameters_against_jsonvalue_then_mapped() {
        ...
        var data = codec.parse(JSON);
        PreparedQuery q = SQL4Json.prepare("SELECT name, age FROM $r WHERE age = :age");
        Person p = q.executeAs(data, Person.class, BoundParameters.named().bind("age", 40));
        assertEquals(new Person("Bob", 40), p);
    }

    @Test
    void when_engine_query_with_parameters_then_works() {
        io.github.mnesimiyilmaz.sql4json.SQL4JsonEngine engine =
                SQL4Json.engine().data(JSON).build();
        String r = engine.query(
                "SELECT name FROM $r WHERE dept = :d", BoundParameters.named().bind("d", "HR"));
        assertTrue(r.contains("Bob"));
        assertFalse(r.contains("Alice"));
    }
    ...
    void when_engine_cache_enabled_and_parameterized_query_then_cache_bypassed() {
        io.github.mnesimiyilmaz.sql4json.settings.Sql4jsonSettings s =
                io.github.mnesimiyilmaz.sql4json.settings.Sql4jsonSettings.builder()
                        ...
                        .build();
        io.github.mnesimiyilmaz.sql4json.SQL4JsonEngine engine =
                SQL4Json.engine().settings(s).data(JSON).build();
        engine.query(
                "SELECT name FROM $r WHERE dept = :d", BoundParameters.named().bind("d", "Eng"));
        engine.query(
                "SELECT name FROM $r WHERE dept = :d", BoundParameters.named().bind("d", "HR"));
        assertEquals(0, engine.cacheSize(), "Parameterised queries must never populate the result cache");
    }
    ...
    void when_searched_case_in_select_uses_positional_param_then_substituted() {
        PreparedQuery q =
                SQL4Json.prepare("SELECT name, CASE WHEN age > ? THEN 'adult' ELSE 'minor' END AS tier FROM $r");
    ...
    void when_simple_case_uses_named_param_in_when_value_then_substituted() {
        ...
        PreparedQuery q =
                SQL4Json.prepare("SELECT name, CASE dept WHEN :d THEN 'primary' ELSE 'other' END AS bucket FROM $r");
    ...
    void array_element_slot_rejects_collection_bind() {
        ...
        SQL4JsonBindException ex = assertThrows(
                SQL4JsonBindException.class,
                () -> pq.execute("[]", BoundParameters.of(java.util.List.of(java.util.List.of("a", "b")))));
        assertTrue(
                ex.getMessage().contains("ARRAY[?]"), "expected message to mention ARRAY[?], was: " + ex.getMessage());
        assertTrue(
                ex.getMessage().toLowerCase().contains("scalar"),
    ...
    void contains_keyword_rejects_collection_bind() {
        ...
        assertTrue(
                ex.getMessage().contains("CONTAINS"), "expected message to mention CONTAINS, was: " + ex.getMessage());
        assertTrue(
                ex.getMessage().toLowerCase().contains("scalar"),
                "expected message to mention 'scalar', was: " + ex.getMessage());
    }
    ...
    void array_operator_accepts_java_array_bind() {
        ...
        String result = SQL4Json.prepare("SELECT id FROM $r WHERE tags @> :req")
                .execute(json, BoundParameters.named().bind("req", new String[] {"admin", "editor"}));
    ...
    void array_operator_with_column_ref_rhs_via_parameter_substitution() {
        ...
        String result = SQL4Json.prepare("SELECT id FROM $r WHERE tags @> required AND id != :ignored")
