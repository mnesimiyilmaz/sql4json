...
/**
 * Exercises {@link ParameterSubstitutor}'s IN-list expansion logic and the scalar-position guard against collections.
 * Uses the substitutor directly + {@link QueryParser}.
 */
class ParameterBindingInExpansionTest {
    ...
    void when_IN_single_placeholder_with_list_then_expanded() {
        ...
        QueryDefinition out = ParameterSubstitutor.substitute(def, BoundParameters.of(List.of(1, 2, 3)), S);
        assertNotNull(out);
    ...
    void when_IN_single_placeholder_with_single_scalar_then_single_element_list() {
        QueryDefinition def = QueryParser.parse("SELECT * FROM $r WHERE id IN (?)", S);
        QueryDefinition out = ParameterSubstitutor.substitute(def, BoundParameters.of(42), S);
        assertEquals(0, out.positionalCount());
    }
    ...
    void when_IN_single_placeholder_with_empty_list_then_ok() {
        ...
        QueryDefinition out = ParameterSubstitutor.substitute(def, BoundParameters.of(List.of()), S);
        assertEquals(0, out.positionalCount());
    }
    ...
    void when_IN_multi_placeholder_classic_arity_works() {
        QueryDefinition def = QueryParser.parse("SELECT * FROM $r WHERE id IN (?, ?)", S);
        QueryDefinition out = ParameterSubstitutor.substitute(def, BoundParameters.of(1, 3), S);
        assertEquals(0, out.positionalCount());
    }
    ...
    void when_IN_expansion_exceeds_max_then_exception() {
        ...
        SQL4JsonBindException e = assertThrows(
                SQL4JsonBindException.class,
                () -> ParameterSubstitutor.substitute(def, BoundParameters.of(List.of(1, 2, 3, 4, 5)), tight));
        assertTrue(
                e.getMessage().toLowerCase().contains("in list"),
    ...
    void when_collection_bound_to_scalar_placeholder_then_exception() {
        ...
        SQL4JsonBindException e = assertThrows(
                SQL4JsonBindException.class,
                () -> ParameterSubstitutor.substitute(def, BoundParameters.of(List.of(1, 2)), S));
        assertTrue(
                e.getMessage().toLowerCase().contains("collection"),
    ...
    void when_array_bound_to_IN_then_expanded() {
        QueryDefinition def = QueryParser.parse("SELECT * FROM $r WHERE id IN (?)", S);
        Integer[] iarr = {1, 2, 3};
        QueryDefinition out = ParameterSubstitutor.substitute(def, BoundParameters.of((Object) iarr), S);
        assertEquals(0, out.positionalCount());
    }

    @Test
    void when_IN_NOT_IN_mixed_with_non_parameterised_scalar_then_ok() {
        QueryDefinition def = QueryParser.parse("SELECT * FROM $r WHERE id NOT IN (?)", S);
        QueryDefinition out = ParameterSubstitutor.substitute(def, BoundParameters.of(List.of(1, 2)), S);
        assertEquals(0, out.positionalCount());
    }
}
