...
class ParameterBindingLimitOffsetTest {
    ...
    void when_named_limit_then_resolved_to_int() {
        ...
        QueryDefinition out = ParameterSubstitutor.substitute(
                def, BoundParameters.named().bind("n", 10), S);
    ...
    void when_limit_bound_null_then_exception() {
        var e = assertThrows(SQL4JsonBindException.class,
                () -> substituteLimit("SELECT * FROM $r LIMIT ?", null));
    ...
    void when_limit_bound_negative_then_exception() {
        assertThrows(SQL4JsonBindException.class,
                () -> substituteLimit("SELECT * FROM $r LIMIT ?", -1));
    }

    @Test
    void when_limit_bound_string_then_exception() {
        assertThrows(SQL4JsonBindException.class,
                () -> substituteLimit("SELECT * FROM $r LIMIT ?", "ten"));
    }

    @Test
    void when_limit_bound_fractional_big_decimal_then_exception() {
        assertThrows(SQL4JsonBindException.class,
                () -> substituteLimit("SELECT * FROM $r LIMIT ?", new BigDecimal("3.14")));
    }
    ...
    void when_limit_bound_big_integer_overflow_then_exception() {
        ...
        assertThrows(SQL4JsonBindException.class,
                () -> substituteLimit("SELECT * FROM $r LIMIT ?", huge));
    }

    @Test
    void when_limit_bound_double_then_exception() {
        assertThrows(SQL4JsonBindException.class,
                () -> substituteLimit("SELECT * FROM $r LIMIT ?", 2.5));
    }

    @Test
    void when_limit_bound_float_then_exception() {
        assertThrows(SQL4JsonBindException.class,
                () -> substituteLimit("SELECT * FROM $r LIMIT ?", 2.5f));
    }
    ...
    void when_limit_bound_long_out_of_int_range_then_exception() {
        ...
        assertThrows(SQL4JsonBindException.class,
                () -> substituteLimit("SELECT * FROM $r LIMIT ?", big));
    }
