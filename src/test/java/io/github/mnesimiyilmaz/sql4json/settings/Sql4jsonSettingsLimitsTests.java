...
class Sql4jsonSettingsLimitsTests {
    ...
    void maxRowsPerQuery_allows_group_by_at_exact_limit() {
        ...
        var settings = Sql4jsonSettings.builder().limits(l -> l.maxRowsPerQuery(3)).build();
    ...
    void maxRowsPerQuery_rejects_group_by_over_limit() {
        ...
        var ex = assertThrows(SQL4JsonExecutionException.class,
                () -> executor.execute(query, data, settings));
    ...
    void maxRowsPerQuery_allows_window_at_exact_limit() {
        ...
        var query = QueryParser.parse(
                "SELECT k, ROW_NUMBER() OVER (ORDER BY k) AS rn FROM $r", settings);
        var data = JsonParser.parse(json.toString());
    ...
    void maxRowsPerQuery_rejects_join_over_limit() {
        ...
        var executor = new QueryExecutor();
        var query = QueryParser.parse(
                "SELECT u.id, o.amt FROM users u JOIN orders o ON u.id = o.userId", settings);
        ...
        var ex = assertThrows(SQL4JsonExecutionException.class,
                () -> executor.execute(query, sources, settings));
    ...
    void maxRowsPerQuery_rejects_streaming_over_limit() {
        ...
        var ex = assertThrows(SQL4JsonExecutionException.class,
                () -> executor.executeStreaming(query, json.toString(), settings));
    ...
    void queryAsJsonValue_jsonvalue_overload_enforces_maxSqlLength() {
        ...
        var ex = assertThrows(SQL4JsonParseException.class,
                () -> SQL4Json.queryAsJsonValue(longSql, data, settings));
        assertTrue(ex.getMessage().contains("SQL query length exceeds configured maximum"));
    }
}
