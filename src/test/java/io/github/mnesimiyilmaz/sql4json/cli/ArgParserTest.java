...
class ArgParserTest {
    ...
    class QueryFlag {
        ...
        void parse_repeatedQuery_throwsUsage() {
            assertThrows(UsageException.class,
                    () -> ArgParser.parse(new String[]{"-q", "a", "-q", "b"}));
        }
    }
    ...
    class FileAndOutput {
        ...
        void parse_fileWithoutValue_throwsUsage() {
            assertThrows(UsageException.class,
                    () -> ArgParser.parse(new String[]{"-q", "x", "-f"}));
        }

        @Test
        void parse_outputWithoutValue_throwsUsage() {
            assertThrows(UsageException.class,
                    () -> ArgParser.parse(new String[]{"-q", "x", "-o"}));
        }
    }
    ...
    class DataSources {
        ...
        void parse_dataMissingEquals_throwsUsage() {
            UsageException ex = assertThrows(UsageException.class,
                    () -> ArgParser.parse(new String[]{"-q", "x", "--data", "noequals"}));
        ...
        void parse_dataEmptyName_throwsUsage() {
            assertThrows(UsageException.class,
                    () -> ArgParser.parse(new String[]{"-q", "x", "--data", "=p.json"}));
        }

        @Test
        void parse_dataEmptyPath_throwsUsage() {
            assertThrows(UsageException.class,
                    () -> ArgParser.parse(new String[]{"-q", "x", "--data", "name="}));
        }
    ...
    class NamedParameters {
        ...
        void parse_paramMalformedJson_throwsUsage() {
            assertThrows(UsageException.class,
                    () -> ArgParser.parse(new String[]{"-q", "x", "-p", "x=not-json"}));
        }

        @Test
        void parse_paramWithoutValue_throwsUsage() {
            assertThrows(UsageException.class,
                    () -> ArgParser.parse(new String[]{"-q", "x", "-p"}));
        }
    }
    ...
    class PrettyAndUnknown {
        ...
        void parse_repeatedPretty_throwsUsage() {
            assertThrows(UsageException.class,
                    () -> ArgParser.parse(new String[]{"-q", "x", "--pretty", "--pretty"}));
        }

        @Test
        void parse_unknownFlag_throwsUsage() {
            UsageException ex = assertThrows(UsageException.class,
                    () -> ArgParser.parse(new String[]{"-q", "x", "--bogus"}));
        ...
        void parse_trailingPositionalArg_throwsUsage() {
            assertThrows(UsageException.class,
                    () -> ArgParser.parse(new String[]{"-q", "x", "stray"}));
        }
