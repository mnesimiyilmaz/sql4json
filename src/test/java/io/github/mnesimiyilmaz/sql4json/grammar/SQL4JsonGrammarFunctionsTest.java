...
class SQL4JsonGrammarFunctionsTest {
    ...
    void everyCategoryExceptValueHasAtLeastOneEntry() {
        Set<Category> present = SQL4JsonGrammar.functions().stream()
                .map(FunctionInfo::category)
    ...
    void arityInvariants() {
        for (FunctionInfo fn : SQL4JsonGrammar.functions()) {
            ...
            assertTrue(fn.maxArity() == -1 || fn.maxArity() >= fn.minArity(),
    ...
    void signatureStartsWithUppercaseName() {
        for (FunctionInfo fn : SQL4JsonGrammar.functions()) {
            assertTrue(fn.signature().startsWith(fn.name().toUpperCase()),
    ...
    void descriptionIsNonEmpty() {
        for (FunctionInfo fn : SQL4JsonGrammar.functions()) {
            assertFalse(fn.description().isBlank(),
                    "empty description for " + fn.name());
        }
    }

    @Test
    void containsKnownSpots() {
        var byName = SQL4JsonGrammar.functions().stream()
                .collect(Collectors.toMap(FunctionInfo::name, f -> f));
