...
class CaseWhenTest {
    ...
    void simpleCaseNumberMatching() {
        var result = arr(
                query("SELECT id, CASE id WHEN 1 THEN 'first' WHEN 2 THEN 'second' ELSE 'other' END AS pos FROM $r"));
        assertEquals(5, result.size());
    ...
    void simpleCaseNoElseReturnsNull() {
        var result = arr(query("SELECT name, CASE dept WHEN 'nonexistent' THEN 'found' END AS x FROM $r"));
        assertEquals(5, result.size());
        for (JsonValue row : result) {
            assertTrue(field(row, "x").isNull(), "expected null when no ELSE and no branch matches");
        }
    }
    ...
    void searchedCaseLike() {
        var result =
                arr(query("SELECT name, CASE WHEN name LIKE 'A%' THEN 'starts_a' ELSE 'other' END AS initial FROM $r"));
        assertEquals(5, result.size());
    ...
    void searchedCaseIsNull() {
        var result = arr(
                query("SELECT name, CASE WHEN status IS NULL THEN 'unknown' ELSE status END AS status_label FROM $r"));
        assertEquals(5, result.size());
    ...
    void caseInSelectWithAlias() {
        var result = arr(query("SELECT CASE WHEN salary > 80000 THEN 'high' ELSE 'low' END AS level FROM $r"));
        assertEquals(5, result.size());
        ...
        for (JsonValue row : result) {
            ...
            var val = str(level);
            assertTrue(val.equals("high") || val.equals("low"), "level should be 'high' or 'low', got: " + val);
        }
    }
    ...
    void caseInWhere() {
        var result =
                arr(query("SELECT name FROM $r WHERE CASE WHEN salary > 80000 THEN 'high' ELSE 'low' END = 'high'"));
        // Alice (90000) and Carol (110000) → 2 rows
        assertEquals(2, result.size());

        var names = result.stream().map(r -> str(field(r, "name"))).toList();
    ...
    void caseInsideFunction() {
        var result = arr(
                query("SELECT name, UPPER(CASE WHEN status = 'active' THEN 'yes' ELSE 'no' END) AS is_active FROM $r"));
        assertEquals(5, result.size());
    ...
    void functionInsideCase() {
        var result =
                arr(query("SELECT name, CASE WHEN LENGTH(name) > 3 THEN 'long' ELSE 'short' END AS name_len FROM $r"));
        assertEquals(5, result.size());
    ...
    void simpleCaseNullNeverMatches() {
        ...
        var result =
                arr(query("SELECT name, CASE status WHEN NULL THEN 'matched' ELSE 'no match' END AS result FROM $r"));
        assertEquals(5, result.size());
    ...
    void nowInSelectReturnsFreshTimestamp() {
        ...
        assertTrue(ts.chars().anyMatch(Character::isDigit), "NOW() result should contain digits: " + ts);
    }

    @Test
    void dateDiffNowWithDateColumn() {
        var result = arr(query("SELECT DATE_DIFF(NOW(), TO_DATE(hire_date), 'DAY') AS days FROM $r LIMIT 1"));
        assertEquals(1, result.size());
        ...
        assertTrue(num(daysField).longValue() > 0, "Days since hire_date should be positive, got: " + num(daysField));
    }
}
