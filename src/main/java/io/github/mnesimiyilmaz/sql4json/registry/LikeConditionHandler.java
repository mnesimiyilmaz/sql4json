...
final class LikeConditionHandler implements ConditionHandler {
    ...
    public CriteriaNode handle(ConditionContext ctx,
        ...
        return buildPatternNode(ctx.lhsExpression(), ctx.rhsExpression(),
                patternCache, functions, false);
    }

    /**
     * Shared LIKE/NOT LIKE pattern matching logic.
     * When {@code negate} is true, the match result is inverted (NOT LIKE).
     */
    static CriteriaNode buildPatternNode(Expression lhs, Expression rhs,
                                         BoundedPatternCache cache, FunctionRegistry functions,
                                         boolean negate) {
     ...
     * <p>
     * ReDoS safety: every literal character (non-wildcard) is wrapped with
     * Pattern.quote() so regex metacharacters (., +, *, [, ], (, ), etc.)
     * are treated as literals. Only % and _ retain their wildcard meaning.
     * <p>
     * Case-insensitive: LIKE comparisons are case-insensitive per SQL convention.
     */
