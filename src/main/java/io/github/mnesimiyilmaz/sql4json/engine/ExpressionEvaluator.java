 ...
 * <ul>
 *   <li>{@link #evaluate} — per-row evaluation of scalar expressions</li>
 *   <li>{@link #evaluateAggregate} — group evaluation: splits at AggregateFnCall,
 *       evaluates inner per-row, aggregates, then applies outer scalars</li>
 * </ul>
 */
public final class ExpressionEvaluator {
     ...
     * Throws if an AggregateFnCall is encountered (use evaluateAggregate instead).
     *
        ...
        return switch (expr) {
            ...
            case SimpleCaseWhen(var subject, var clauses, var elseExpr) -> {
                ...
                for (var wc : clauses) {
                    ...
                    if (!subjectVal.isNull() && !whenVal.isNull()
    ...
    /**
     * Evaluate an expression that may contain an aggregate, against a group of rows.
     * Splits at the AggregateFnCall: evaluates inner per-row, aggregates,
     * then applies any outer scalar functions to the result.
     *
