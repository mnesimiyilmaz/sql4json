 ...
 *
 * <p>Consumes the parser-collected list of {@link Expression.WindowFnCall} nodes
 * and an alias map derived from SELECT columns whose top-level expression IS a
 * {@code WindowFnCall}. Builds a window-aware {@link RowSchema} via
 * {@link RowSchema#withWindowSlots(List, Map)} — the alias becomes the canonical
 * column key for the slot, so {@code ORDER BY alias} / any {@code ColumnRef}
 * lookup against the alias resolves naturally through the schema index.
 *
 * <p>For each row, the stage allocates an {@code Object[]} working buffer sized
 * to {@code schema.size()}, prefills with input values, and writes window
 * results into the appropriate slot ordinals. Each buffer is wrapped in a
 * {@link FlatRow} for the downstream {@code Stream<RowAccessor>} pipeline.
 *
 * <p>Wrapped windows like {@code ROUND(... OVER (...))} get a synthetic column
 * key and resolve through {@link RowAccessor#getWindowResult(Expression.WindowFnCall)}
 * → {@link RowSchema#windowSlot(Expression.WindowFnCall)}.
 *
 * @since 1.2.0
 */
public final class WindowStage implements MaterializingPipelineStage {
     ...
     *
     * @param windowCalls       every {@link Expression.WindowFnCall} the parser saw, in
     *                          source order (typically {@code QueryDefinition.windowFunctionCalls()})
     * @param selectedColumns   SELECT column definitions; used to derive an alias-{@link FieldKey}
     *                          mirror for plain windowed columns so that ORDER BY / HAVING / any
     *                          {@code ColumnRef} lookup against the alias resolves to the
     ...
     *                          {@code QueryDefinition.referencedColumns()}; used as the input-side
     *                          schema for non-asterisk queries to avoid an O(N×K) scan over input
     *                          rows
     ...
     * @since 1.2.0
     */
    public WindowStage(List<Expression.WindowFnCall> windowCalls,
    ...
    public Stream<RowAccessor> apply(Stream<RowAccessor> input) {
        ...
        RowSchema schema = baseSchema.withWindowSlots(
                new ArrayList<>(distinctCalls), aliasKeysByCall);
    ...
    private Object[][] prefillWorkingBuffers(List<RowAccessor> allRows, RowSchema schema,
                                             RowSchema baseSchema) {
    ...
    private void processWindowSpec(WindowSpec spec, List<Expression.WindowFnCall> calls,
                                   List<RowAccessor> allRows, Object[][] working, RowSchema schema) {
    ...
    private List<RowAccessor> wrapWorkingBuffers(List<RowAccessor> allRows, RowSchema schema,
                                                 Object[][] working) {
    ...
    private static FlatRow wrapBuffer(RowAccessor src, RowSchema schema, Object[] vals) {
        if (src.isAggregated()) {
            return FlatRow.aggregated(schema, vals,
                    src.sourceGroup().orElseGet(Collections::emptyList));
        }
        return FlatRow.of(schema, vals);
    }

    /**
     * Collects the input-side schema for the materialization buffer. Three paths in
     * descending order of cost-savings:
     *
     * <ol>
     *   <li><b>FlatRow input (post-GROUP BY / post-JOIN / post-engine pre-flatten):</b>
     *       Schema is uniform across the stream — return the first row's schema in O(1).</li>
     *   <li><b>Non-{@code SELECT *} {@link Row} input:</b> use the parser-collected
     *       {@code referencedColumns} set, which already covers every {@link Expression.ColumnRef}
     *       seen in WHERE / GROUP BY / HAVING / ORDER BY / SELECT / WINDOW partition+order.
     *       O(K_ref). Avoids the per-row full-flatten that {@link Row#keys()} triggers on lazy
     *       rows.</li>
     *   <li><b>{@code SELECT *} {@link Row} input:</b> referenced fields can't be statically
     *       enumerated, so we scan every row's keys. O(N×K) — only on the explicit asterisk path.</li>
     * </ol>
     */
    ...
    private void sortIndices(List<Integer> partition, List<OrderByColumnDef> orderBy,
                             List<RowAccessor> rows) {
        Comparator<Integer> cmp = orderBy.stream()
                .map(col -> {
                    Comparator<Integer> c = Comparator.comparing(
                            (Integer idx) -> ExpressionEvaluator.evaluate(
                                    col.expression(), rows.get(idx), functionRegistry),
    ...
    private void evaluateWindowFunction(Expression.WindowFnCall wfc, List<Integer> partition,
                                        List<RowAccessor> rows, Object[][] working, int slot) {
    ...
    private void computeRank(Expression.WindowFnCall wfc, List<Integer> partition,
                             List<RowAccessor> rows, Object[][] working, int slot) {
        int prevRank = 0;
        for (int i = 0; i < partition.size(); i++) {
            int rank;
    ...
    private void computeDenseRank(Expression.WindowFnCall wfc, List<Integer> partition,
                                  List<RowAccessor> rows, Object[][] working, int slot) {
        int currentRank = 0;
        for (int i = 0; i < partition.size(); i++) {
    ...
    private void computeNtile(Expression.WindowFnCall wfc, List<Integer> partition,
                              List<RowAccessor> rows, Object[][] working, int slot) {
        if (wfc.args().isEmpty()) {
            throw new SQL4JsonExecutionException("NTILE requires a bucket count argument");
        }
        SqlValue arg = ExpressionEvaluator.evaluate(
                wfc.args().getFirst(), rows.get(partition.getFirst()), functionRegistry);
    ...
    private void computeLag(Expression.WindowFnCall wfc, List<Integer> partition,
                            List<RowAccessor> rows, Object[][] working, int slot) {
    ...
    private void computeLead(Expression.WindowFnCall wfc, List<Integer> partition,
                             List<RowAccessor> rows, Object[][] working, int slot) {
        int offset = resolveOffset(wfc);
        Expression argExpr = wfc.args().getFirst();
        for (int i = 0; i < partition.size(); i++) {
    ...
    private int resolveOffset(Expression.WindowFnCall wfc) {
        if (wfc.args().size() > 1) {
            SqlValue offsetVal = ExpressionEvaluator.evaluate(
                    wfc.args().get(1), Row.eager(Map.of()), functionRegistry);
            return (int) ((SqlNumber) offsetVal).longValue();
        }
        return 1;
    }

    private void computeAggregateWindow(String aggName, Expression.WindowFnCall wfc,
                                        List<Integer> partition, List<RowAccessor> rows,
                                        Object[][] working, int slot) {
