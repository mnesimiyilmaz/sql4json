...
/**
 * Walks a {@link QueryDefinition} and replaces every {@link Expression.ParameterRef} with a resolved
 * {@link Expression.LiteralVal}. Returns a new immutable {@code QueryDefinition}.
 *
 * <p>Thread-safe: stateless, per-call construction of the substituted definition. The original {@code QueryDefinition}
 * is never mutated.
 */
public final class ParameterSubstitutor {
    ...
    /**
     * Returns a {@link QueryDefinition} with every {@code ParameterRef} replaced by a resolved literal. Walks SELECT,
     * WHERE, GROUP BY, HAVING, ORDER BY, and LIMIT / OFFSET.
     *
     * <p>Queries that declared no placeholders are returned unchanged (fast-path) so repeat execution avoids any
     * re-resolution cost.
     *
     * @param def the parsed query definition (may contain {@code ParameterRef} nodes)
     * @param params the bound parameter values
     * @param settings the settings (used to look up the {@link ConditionHandlerRegistry} for re-resolving substituted
     *     condition contexts)
     ...
     */
    public static QueryDefinition substitute(QueryDefinition def, BoundParameters params, Sql4jsonSettings settings) {
     ...
     * @param params the bound parameter values
     * @param settings the settings (used to resolve the {@link ConditionHandlerRegistry} when walking
     *     {@link Expression.SearchedCaseWhen} conditions)
     * @return the substituted expression tree; the same instance is returned when no {@code ParameterRef} node is
     *     encountered (for {@code LiteralVal}, {@code ColumnRef}, {@code NowRef}, and {@code COUNT(*)})
     * @throws SQL4JsonBindException on bind failure
     */
    public static Expression substituteExpression(Expression expr, BoundParameters params, Sql4jsonSettings settings) {
        return switch (expr) {
    ...
    private static SelectColumnDef substituteSelectColumn(
            SelectColumnDef sc, BoundParameters params, Sql4jsonSettings settings) {
        if (sc.isAsterisk()) {
            return sc;
        }
    ...
    private static CriteriaNode substituteCriteria(
            CriteriaNode node, BoundParameters params, Sql4jsonSettings settings, ConditionHandlerRegistry registry) {
        return switch (node) {
            case null -> null;
    ...
    private static ConditionContext substituteConditionContext(
        ...
        SqlValue newTestValue = newRhsExpr instanceof Expression.LiteralVal(SqlValue v) ? v : cc.testValue();
    ...
    /**
     * CONTAINS — scalar RHS, but with a clearer error message when the caller accidentally binds a Collection (a common
     * mistake for users migrating from PostgreSQL's {@code @>}). Without this guard the bind would fall through
     * {@code resolveParam} and surface the generic "scalar placeholder" message that doesn't mention CONTAINS, leaving
     * the user to guess which operator complained.
     */
    private static void rejectCollectionBindForContains(ConditionContext cc, BoundParameters params) {
        ...
        if (raw instanceof java.util.Collection<?>
            ...
            throw new SQL4JsonBindException("CONTAINS expects a scalar bind for parameter " + paramLabel(p)
                    + "; got Collection. Use @>, <@, &&, or = with a list/array bind"
                    + " for whole-array operators.");
        }
    }

    private static Expression substituteOrNull(Expression expr, BoundParameters params, Sql4jsonSettings settings) {
        return expr == null ? null : substituteExpression(expr, params, settings);
    }

    /**
     * Substitutes a BETWEEN bound expression. Returns the resolved literal in {@link SubstitutedBound#literal()} when
     * substitution reduces to a {@code LiteralVal}; otherwise the substituted expression survives in
     * {@link SubstitutedBound#expression()} for per-row evaluation by {@code BetweenConditionHandler}.
     */
    private static SubstitutedBound substituteBound(
            Expression boundExpr, SqlValue defaultLiteral, BoundParameters params, Sql4jsonSettings settings) {
        if (boundExpr == null) {
     ...
     *
     * <ul>
     *   <li>{@code Byte}, {@code Short}, {@code Integer}, {@code Long} — accepted if within {@code [0,
     *       Integer.MAX_VALUE]}.
     *   <li>{@code BigInteger} — accepted via {@code longValueExact()}; overflow throws.
     *   <li>{@code BigDecimal} with scale &le; 0 after {@code stripTrailingZeros()} and within int range — accepted;
     *       non-zero fractional part throws.
     *   <li>{@code Double}, {@code Float}, or any other {@code Number} subtype — rejected (non-integer numeric type).
     *   <li>{@code null} or non-{@code Number} — rejected.
     *   <li>Negative values or values {@code > Integer.MAX_VALUE} — rejected.
     * </ul>
     *
     ...
     */
    private static Integer resolveLimitOffset(Expression.ParameterRef ref, BoundParameters params, String clause) {
    ...
    /**
     * Builds the printable label for a {@link Expression.ParameterRef} — {@code ":name"} for named placeholders,
     * {@code "position N"} for positional ones. Used in bind-time error messages.
     */
    ...
    /**
     * Builds a position-aware error message when a Collection / array is bound to a scalar placeholder slot.
     * {@link ParameterPositionKind#ARRAY_ELEMENT} ({@code ARRAY[?]}) gets a hint pointing at the bare-RHS form;
     * everything else falls back to the generic message. {@code BARE_ARRAY_RHS} and {@code IN_LIST} are not routed
     * through {@code resolveParam} (they go through {@code expandArrayParameter} / {@code expandInListElement}); the
     * branch exists only as a safety net so an unexpected dispatch doesn't yield a misleading message.
     *
    ...
    private static String rejectCollectionMessage(Expression.ParameterRef p, String label) {
        return switch (p.positionKind()) {
            case ARRAY_ELEMENT ->
                "Cannot bind Collection to ARRAY[?] slot at " + label
    ...
    /**
     * Expands an IN / NOT IN value-expression list into a flat {@code List<SqlValue>}, honouring collection / array
     * expansion for {@code ParameterRef} elements.
     *
     * <p>For each value expression:
     *
     * <ul>
     *   <li>{@code LiteralVal} — its {@code SqlValue} is appended.
     *   <li>{@code ParameterRef} — the raw Java value is read; a {@link java.util.Collection} or array expands into one
     *       {@code SqlValue} per element, anything else becomes a single element. This is the <b>only</b> context in
     *       which collections are permitted.
     *   <li>Any other expression — substituted recursively and must reduce to a literal.
     * </ul>
     *
     * <p>The post-expansion size is checked against
     * {@link io.github.mnesimiyilmaz.sql4json.settings.LimitsSettings#maxInListSize()}. Exceeding it throws
     * {@link SQL4JsonBindException}.
     *
     * @param valueExprs the condition's value-expression list (non-null)
     * @param params the bound parameter values
     * @param settings the settings supplying the limits
     * @param lhsForContext the LHS expression of the enclosing condition (for error messages; may be {@code null})
     * @return an immutable flat list of resolved {@code SqlValue}s
     * @throws SQL4JsonBindException on bind failure or limit violation
     */
    private static List<SqlValue> expandInList(
            List<Expression> valueExprs, BoundParameters params, Sql4jsonSettings settings, Expression lhsForContext) {
        ...
        if (out.size() > max) {
            ...
            throw new SQL4JsonBindException(
                    "IN list size after expansion exceeds configured maximum (" + max + ") at " + fieldLabel);
        }
        return List.copyOf(out);
    }

    private static void expandInListElement(
            Expression e, BoundParameters params, Sql4jsonSettings settings, List<SqlValue> out) {
     ...
     *
     * <ul>
     *   <li>{@code valueExpressions} is non-null (parsed {@code ARRAY[…]} literal) — each element expression is
     *       substituted recursively; the resulting list stays in {@code valueExpressions}.
     *   <li>{@code rhsExpression} is a {@link Expression.ParameterRef} (bare {@code ?} / {@code :name}) — the bound
     *       value must be a {@link java.util.Collection} or array; each element is wrapped into a
     *       {@link Expression.LiteralVal} and the flat list lands in {@code valueExpressions}. The
     *       {@code rhsExpression} field is cleared.
     *   <li>{@code rhsExpression} is anything else (column-ref) — substituted recursively and kept in
     *       {@code rhsExpression}.
     * </ul>
     */
    private static ConditionContext substituteArrayPredicate(
        ...
        Expression newLhs =
                cc.lhsExpression() == null ? null : substituteExpression(cc.lhsExpression(), params, settings);
        ...
        if (cc.valueExpressions() != null) {
            List<Expression> newElements = cc.valueExpressions().stream()
                    ...
                    .toList();
            return new ConditionContext(
                    cc.type(), newLhs, cc.operator(), null, null, null, null, null, newElements, null, null);
        }
        ...
        if (rhs instanceof Expression.ParameterRef p) {
            ...
            return new ConditionContext(
                    cc.type(), newLhs, cc.operator(), null, null, null, null, null, elements, null, null);
        }
        ...
        return new ConditionContext(cc.type(), newLhs, cc.operator(), null, newRhs, null, null, null, null, null, null);
    }

    /**
     * Expands a {@link java.util.Collection} or array bound to an array-operator parameter into a list of
     * {@link Expression.LiteralVal} entries (one per element, converted via
     ...
     */
    private static List<Expression> expandArrayParameter(Object raw, Expression.ParameterRef p, String operator) {
        String label = paramLabel(p);
        if (raw == null) {
            throw new SQL4JsonBindException(
                    "array operator '" + operator + "' parameter " + label + " bound to null; expected a Collection");
        }
        ...
        if (raw.getClass().isArray()) {
            ...
            for (int i = 0; i < len; i++) {
                out.add(new Expression.LiteralVal(ParameterConverter.toSqlValue(java.lang.reflect.Array.get(raw, i))));
            }
            return List.copyOf(out);
        }
        throw new SQL4JsonBindException("array operator '" + operator + "' parameter " + label
    ...
    private static Expression.AggregateFnCall substituteAggregate(
        ...
        return new Expression.AggregateFnCall(agg.name(), substituteExpression(agg.inner(), params, settings));
    }
    ...
    private static WindowSpec substituteWindowSpec(WindowSpec spec, BoundParameters params, Sql4jsonSettings settings) {
        ...
        List<OrderByColumnDef> newOrder = spec.orderBy().stream()
                .map(o -> OrderByColumnDef.of(substituteExpression(o.expression(), params, settings), o.direction()))
                .toList();
    ...
    private static Expression.SimpleCaseWhen substituteSimpleCase(
        ...
        List<Expression.WhenClause.ValueWhen> newClauses =
                new ArrayList<>(sc.whenClauses().size());
        ...
        Expression newElse = sc.elseExpr() == null ? null : substituteExpression(sc.elseExpr(), params, settings);
    ...
    private static Expression.SearchedCaseWhen substituteSearchedCase(
        ...
        List<Expression.WhenClause.SearchWhen> newClauses =
                new ArrayList<>(sc.whenClauses().size());
    ...
    private static List<Expression> substituteAll(
            List<Expression> list, BoundParameters params, Sql4jsonSettings settings) {
