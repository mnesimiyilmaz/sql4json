...
/**
 * Converts a Java {@link Object} parameter value into an internal {@link SqlValue} suitable for substitution into a
 * {@code ParameterRef}. Supports the common Java types users will hand to {@code PreparedQuery.execute(...)} and all
 * the SqlValue subtypes themselves (for passthrough when the caller binds a pre-built {@code SqlValue}).
 *
 * <p>Supported types:
 *
 * <ul>
 *   <li>{@link String}
 *   <li>All primitive numeric wrappers: {@code Byte, Short, Integer, Long, Float, Double}
 *   <li>{@link java.math.BigDecimal}, {@link java.math.BigInteger}, other {@link Number}
 *   <li>{@link Boolean}
 *   <li>{@link LocalDate}, {@link LocalDateTime}
 *   <li>{@link Instant}, {@link ZonedDateTime}, {@link OffsetDateTime} — normalised to UTC local-datetime
 *   <li>{@link java.util.Date}
 *   <li>Already-wrapped {@link SqlValue} — returned as is
 *   <li>{@code null} → {@link SqlNull#INSTANCE}
 * </ul>
 *
    ...
    public static SqlValue toSqlValue(Object value) {
        return switch (value) {
            ...
            case ZonedDateTime zdt ->
                new SqlDateTime(zdt.withZoneSameInstant(ZoneOffset.UTC).toLocalDateTime());
            case OffsetDateTime odt ->
                new SqlDateTime(odt.withOffsetSameInstant(ZoneOffset.UTC).toLocalDateTime());
