...
/**
 * Immutable parsed query representation produced by QueryParser. Consumed by QueryPipeline.
 *
 ...
 * @param subqueryPositionalOffset the global offset to pass when re-parsing the inner subquery (0 for top-level parse
 *     without subqueries)
 * @param windowFunctionCalls every {@link Expression.WindowFnCall} built by the parser, in source order. Includes calls
 *     buried inside {@link CriteriaNode} closures (CASE WHEN conditions). Consumed by WindowStage to precompute results
 *     stored on each row.
 */
public record QueryDefinition(
    ...
    /**
     * True iff any window function appears anywhere in the query — triggers WindowStage in the pipeline. Reflects the
     * parser-collected list, which catches windows buried inside CASE WHEN conditions (where the {@link CriteriaNode}
     * closure is opaque to tree-walks).
     *
     ...
     * {@link io.github.mnesimiyilmaz.sql4json.engine.FieldKey}s. Used by materialization stages to derive a
     * {@link io.github.mnesimiyilmaz.sql4json.engine.RowSchema} without an extra pass over input rows.
     *
