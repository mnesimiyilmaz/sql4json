...
/**
 * Cached reflection metadata for a mapping target. Either a record (canonical constructor + components) or a POJO
 * (no-arg constructor + setter map).
 *
 * <p>Instances are built once per {@link Class} and stored in {@link JsonValueMapper}'s {@code ConcurrentHashMap}
 * cache.
 */
sealed interface TypeDescriptor permits TypeDescriptor.RecordDescriptor, TypeDescriptor.PojoDescriptor {
    ...
    /**
     * @throws SQL4JsonMappingException if {@code type} is neither a record nor a POJO with a public no-arg constructor
     */
    ...
    record RecordDescriptor(
        ...
        static RecordDescriptor build(Class<?> type) {
            ...
            try {
                ...
                throw new SQL4JsonMappingException("Cannot find canonical constructor for record " + type.getName(), e);
            }
        }
    }

    record PojoDescriptor(Class<?> type, Constructor<?> noArgConstructor, Map<String, SetterInfo> settersByProperty)
            implements TypeDescriptor {
        ...
        static PojoDescriptor build(Class<?> type) {
            ...
            try {
                ...
                throw new SQL4JsonMappingException("No public no-arg constructor for " + type.getName(), e);
            }
            ...
            setterList.sort(
                    Comparator.comparing(Method::getName).thenComparing(m -> m.getParameterTypes()[0].getName()));
            ...
            for (Method m : setterList) {
                ...
                if (existing == null
                        || isMoreSpecific(
                                m.getParameterTypes()[0], existing.method.getParameterTypes()[0])) {
