...
/**
 * Per-{@code map()}-call cycle detector. Tracks identities currently on the mapping stack, scoped to POJO targets
 * (records can't cycle because Java prevents self-reference in record component types). The identity key is any object
 * whose reference uniquely represents the frame — typically the underlying field map of a {@code JsonObjectValue}.
 *
