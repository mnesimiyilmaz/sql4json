...
/**
 * Flattens nested {@link JsonValue} trees into flat key-value rows for SQL processing,
 * and provides path navigation within JSON structures.
 */
public final class JsonFlattener {
    ...
    /**
     * Primary: Stream of lazy rows — each row holds its original JsonValue reference.
     * No upfront flatten. Fields are resolved on demand via Row.get().
     *
     ...
     */
    public static Stream<Row> streamLazy(JsonValue root, String rootPath,
     ...
     * @param interner the field key interner for deduplicating keys
     */
    public static void flattenInto(JsonValue element, Map<FieldKey, SqlValue> target,
                                   FieldKey.Interner interner) {
        flattenInto(element, null, target, interner);
    }

    /**
     * Full flatten with optional alias prefix.
     * When prefix is non-null, all field keys are prefixed: "alias.fieldName".
     ...
     * @param interner the field key interner for deduplicating keys
     */
    public static void flattenInto(JsonValue element, String prefix,
                                   Map<FieldKey, SqlValue> target,
                                   FieldKey.Interner interner) {
    ...
    /**
     * Navigate from root to the path specified in the FROM clause.
     * "$r" or null returns root. "$r.data.items" navigates to root.data.items.
     *
    ...
    private static void flattenRecursive(JsonValue value, StringBuilder path,
                                         Map<FieldKey, SqlValue> result,
                                         FieldKey.Interner interner) {
        switch (value) {
