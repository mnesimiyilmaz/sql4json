...
/**
 * Built-in {@link JsonCodec} backed by the library's own JSON parser and serializer. This is the default used when no
 * custom codec is provided.
 */
...
public final class DefaultJsonCodec implements JsonCodec {
     ...
     *
     * <p>Use this constructor when the default security limits need to be adjusted — for example, to accept larger
     * inputs or to change the {@link DefaultJsonCodecSettings#duplicateKeyPolicy()}. The supplied settings are applied
     * by both {@code JsonParser} and {@code StreamingJsonParser} during every parse operation.
     *
     * <p>Usage example:
     *
     * <pre>{@code
     ...
     * }</pre>
     *
     * @param settings non-null codec settings; use {@link DefaultJsonCodecSettings#defaults()} for the default safe
     *     limits
